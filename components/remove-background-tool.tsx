...
export function RemoveBackgroundTool() {
  ...
  const select = async (next?: File) => {
    if (!next) return
    if (!accepted.includes(next.type)) { setError("Choose a PNG, JPG, JPEG, or WEBP image."); setState("error"); return }
    setError(""); setState("processing")
    try { setResult(await removeBackground(next)); setState("success") } catch { setError("We couldn’t process that image. Try another supported format."); setState("error") }
  }
